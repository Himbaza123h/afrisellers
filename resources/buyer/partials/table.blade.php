                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            @if($buyer->email_verified)
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                                    <i class="fas fa-check-circle mr-1.5 text-[10px]"></i>Verified
                                </span>
                            @else
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                {{-- View --}}
                                <a href="{{ route('admin.buyer.show', $buyer) }}"
                                   class="p-1.5 text-gray-600 rounded-lg hover:bg-blue-50 hover:text-blue-600"
                                   title="View Details">
                                    <i class="fas fa-eye text-sm"></i>
                                </a>

                                {{-- Activate (only when pending) --}}
                                @if($buyer->account_status === 'pending')
                                    <form action="{{ route('admin.buyer.update-status', $buyer->id) }}" method="POST" class="inline">
                                        @csrf
                                        ...
                                        <button type="submit"
                                                ...
                                                title="Activate"
                                                onclick="return confirm('Activate this buyer account?')">
                                ...
                                @if($buyer->account_status === 'active')
                                    <form action="{{ route('admin.buyer.update-status', $buyer->id) }}" method="POST" class="inline">
                                        ...
                                        <button type="submit"
                                                class="p-1.5 text-gray-600 rounded-lg hover:bg-orange-50 hover:text-orange-600"
                                                title="Suspend"
                                                onclick="return confirm('Suspend this buyer account?')">
                                ...
                                <div class="relative inline-block text-left">
                                    <button type="button"
                                            class="p-1.5 text-gray-600 rounded-lg hover:bg-gray-100"
                                            onclick="toggleDropdown(event, 'dropdown-{{ $buyer->id }}')">
                                        <i class="fas fa-ellipsis-v text-sm"></i>
                                    </button>
                                    <div id="dropdown-{{ $buyer->id }}"
                                         class="hidden absolute right-0 mt-2 w-56 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10">
                                        <div class="py-1">

                                            {{-- Reactivate (only when suspended) --}}
                                            @if($buyer->account_status === 'suspended')
                                                <form action="{{ route('admin.buyer.update-status', $buyer->id) }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="account_status" value="active">
                                                    <button type="submit"
                                                            class="w-full flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 text-left"
                                                            onclick="return confirm('Reactivate this buyer account?')">
                                            ...
                                            {{-- Verify Email (placeholder — wire up when route exists) --}}
                                          @if(!$buyer->email_verified)
    <form action="{{ route('admin.buyer.verify-email', $buyer->id) }}" method="POST">
        @csrf
        <button type="submit"
                class="w-full flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 text-left"
                onclick="return confirm('Manually verify this buyer\'s email?')">
            <i class="fas fa-envelope text-emerald-600 w-4"></i>
            Verify Email
        </button>
    </form>
@endif
                                            ...
                                            <button type="button"
                                                    onclick="dashboardSwitch.open({{ $buyer->id }}, '{{ addslashes($buyer->user->name ?? 'Buyer') }}', 'Buyer', '{{ route('admin.buyer.switch-to-buyer', $buyer) }}')"
                                                    class="w-full flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 text-left">
        ...
        <div class="px-4 py-3 border-t bg-gray-50">
            <div class="flex items-center justify-between">
                <span class="text-sm text-gray-700">
                    Showing {{ $buyers->firstItem() }}-{{ $buyers->lastItem() }} of {{ $buyers->total() }}
                </span>
