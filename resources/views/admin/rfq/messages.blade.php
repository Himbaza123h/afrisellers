...
<div class="px-4 py-6 mx-auto max-w-7xl  sm:px-6 lg:px-8">
    ...
    <div class="mb-6">
        <div class="flex gap-3 items-center mb-4">
            <a href="{{ route('admin.rfq.vendors', $rfq) }}" class="text-gray-600 hover:text-gray-900">
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <div class="p-4 space-y-4 max-h-[500px] overflow-y-auto">
            ...
            @forelse($messages as $message)
                <div class="flex gap-3 {{ $message->sender_type === 'buyer' ? '' : ($message->sender_type === 'admin' ? 'flex-row-reverse' : 'flex-row-reverse') }}">
                    <div class="w-8 h-8 {{ $message->sender_type === 'buyer' ? 'bg-blue-500' : ($message->sender_type === 'admin' ? 'bg-[#ff0808]' : 'bg-[#ff0808]') }} rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                    ...
                    <div class="flex-1 {{ $message->sender_type === 'buyer' ? '' : 'text-right' }}">
                        ...
                        <div class="inline-block {{ $message->sender_type === 'buyer' ? 'bg-blue-50 border border-blue-200' : ($message->sender_type === 'admin' ? 'bg-[#ff0808] text-white border border-red-600' : 'bg-gray-50 border border-gray-200') }} rounded-lg p-3">
                            <p class="text-sm {{ $message->sender_type === 'admin' ? 'text-white' : 'text-gray-700' }} whitespace-pre-wrap">{{ $message->message }}</p>
                        </div>
                    </div>
                </div>
            @empty
            ...
            <div class="p-4 border-t border-gray-200">
                ...
                @else
                    <form action="{{ route('admin.rfq.message.store', $rfq) }}" method="POST">
                        @csrf
