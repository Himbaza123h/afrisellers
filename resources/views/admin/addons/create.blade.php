...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Create Addon</h1>
            <p class="mt-1 text-sm text-gray-500">Add a new promotional addon placement</p>
        </div>
    </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.addons.store') }}" method="POST" class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="number" name="price" value="{{ old('price') }}" step="0.01" min="0" max="999999.99" placeholder="0.00" class="w-full pl-8 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500" required>
            </div>
            @error('price')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
