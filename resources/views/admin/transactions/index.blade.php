...
<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Transactions Management</h1>
            <p class="mt-1 text-xs text-gray-500">Monitor and manage all financial transactions</p>
        </div>
        <div class="flex flex-wrap gap-2 no-print">
            <button onclick="window.open('{{ route('admin.transactions.print') }}', '_blank')" class="inline-flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all font-medium shadow-sm text-sm">
                <i class="fas fa-print"></i>
                <span>Print</span>
            </button>
            <form action="{{ route('admin.transactions.export') }}" method="POST" class="inline">
                @csrf
            </form>
        </div>
    </div>
    ...
    @if(session('info'))
        <div class="p-3 bg-blue-50 rounded-lg border border-blue-200 flex items-start gap-3 no-print">
    ...
    <div id="stats-section" class="stats-container">
        @include('admin.transactions.partials.stats')
    </div>
...
function openRefundModal(transactionId, amount) {
    ...
    const maxRefund = document.getElementById('maxRefund');
    const refundAmount = document.getElementById('refundAmount');

    form.action = `/admin/transactions/${transactionId}/refund`;
    maxRefund.textContent = parseFloat(amount).toFixed(2);
    refundAmount.max = amount;
    refundAmount.value = amount; // Pre-fill with full amount
...
function openStatusModal(transactionId) {
    ...
    form.action = `/admin/transactions/${transactionId}/update-status`;
    modal.classList.remove('hidden');
}
...
document.getElementById('refundModal')?.addEventListener('click', function(event) {
    if (event.target === this) closeRefundModal();
});

document.getElementById('statusModal')?.addEventListener('click', function(event) {
    if (event.target === this) closeStatusModal();
});
