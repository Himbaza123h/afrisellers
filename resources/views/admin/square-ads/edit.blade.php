...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Edit Square Ad</h1>
            <p class="text-xs text-gray-500 mt-0.5">Update the square ad details</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.square-ads.update', $squareAd) }}" method="POST"
        ...
        <div>
            ...
            <input type="hidden" name="library_id" id="library_id" value="{{ old('library_id', $squareAd->library_id) }}" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                @php $selectedType = old('type', $squareAd->type); @endphp
                <select name="type" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#ff0808]">
                    <option value="">-- Select Type --</option>
                    <option value="Weekly Special Offers" {{ $selectedType == 'Weekly Special Offers' ? 'selected' : '' }}>Weekly Special Offers</option>
                    <option value="Hot Deals" {{ $selectedType == 'Hot Deals' ? 'selected' : '' }}>Hot Deals</option>
                    <option value="The Popular Suppliers" {{ $selectedType == 'The Popular Suppliers' ? 'selected' : '' }}>Most Recommended Sales</option>
                    {{-- <option value="The Popular Suppliers" {{ $selectedType == 'The Popular Suppliers' ? 'selected' : '' }}>The Popular Suppliers</option> --}}
                    <option value="Trending Products" {{ $selectedType == 'Trending Products' ? 'selected' : '' }}>Trending Products</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Badge Text</label>
                <input type="text" name="badge" value="{{ old('badge', $squareAd->badge) }}" maxlength="50"
        ...
        <div>
            ...
            <input type="text" name="headline" value="{{ old('headline', $squareAd->headline) }}" required maxlength="255"
                   class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#ff0808]">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Sub Text</label>
            <input type="text" name="sub_text" value="{{ old('sub_text', $squareAd->sub_text) }}" maxlength="255"
                   class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#ff0808]">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div>
                ...
                <input type="url" name="cta_url" value="{{ old('cta_url', $squareAd->cta_url) }}"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#ff0808]">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Accent Color</label>
                <div class="flex gap-2">
                    <input type="text" name="accent" id="accentText" value="{{ old('accent', $squareAd->accent) }}"
                    ...
                    <input type="color" id="accentPicker" value="{{ old('accent', $squareAd->accent) }}"
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $squareAd->sort_order) }}" min="0"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#ff0808]">
            </div>
        </div>
...
const currentId = '{{ $squareAd->library_id }}';
if (currentId) {
    const el = document.querySelector(`.library-item[data-id="${currentId}"]`);
    if (el) selectMedia(el, parseInt(currentId));
}
</script>
@endsection
