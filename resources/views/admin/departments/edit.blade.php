...
<div class="max-w-2xl mx-auto px-4 py-5">
    <div class="mb-5">
        <div class="flex items-center gap-3 mb-1">
            ...
            <h1 class="text-xl font-bold text-gray-900">Edit Department</h1>
        </div>
        <p class="text-xs text-gray-500 ml-11">Editing <strong>{{ $department->name }}</strong></p>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff0808] focus:border-transparent">{{ old('description', $department->description) }}</textarea>
            </div>

            <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Color</label>
                <div class="flex items-center gap-3">
                    <input type="color" name="color" value="{{ old('color', $department->color) }}"
                           class="h-9 w-16 rounded-lg border border-gray-300 cursor-pointer">
                    <span class="text-xs text-gray-500">Used to visually identify this department</span>
                </div>
            </div>
