                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    <div class="w-12 h-12 bg-gray-100 to-gray-200 rounded-lg flex items-center justify-center border border-gray-200">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    {{-- Section badges — show which sections this product is manually pinned to --}}
                                    @php
                                        $pinnedSections = \App\Models\UISection::all()->filter(function($sec) use ($product) {
                                            return collect($sec->manual_items ?? [])->contains(fn($i) => (int)$i['id'] === $product->id && $i['type'] === 'product');
                                        });
                                    @endphp
                                    @if($pinnedSections->count())
                                        <div class="flex flex-wrap gap-1 mt-1">
                                            @foreach($pinnedSections as $sec)
                                                <span class="inline-flex items-center gap-0.5 text-[9px] font-semibold text-purple-700 bg-purple-50 border border-purple-200 px-1.5 py-0.5 rounded-full">
                                                    <i class="fas fa-layer-group" style="font-size:7px"></i>
                                                    {{ \Illuminate\Support\Str::limit($sec->name, 18) }}
                                                </span>
                                            @endforeach
                                        </div>
                                    @endif
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-1.5 flex-wrap">
                                ...
                                <a href="{{ route('admin.product.show', $product) }}"
                                   class="p-2 text-gray-600 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors"
                                   title="View">
                                ...
                                @if(auth()->user()->isVendor() && !auth()->user()->hasRole('admin'))
                                    ...
                                    <a href="{{ route('admin.product.edit', $product) }}"
                                       class="p-2 text-gray-600 rounded-lg hover:bg-yellow-50 hover:text-yellow-600 transition-colors"
                                       title="Edit">
                                    ...
                                    <form action="{{ route('admin.product.destroy', $product) }}" method="POST" class="inline"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                class="p-2 text-gray-600 rounded-lg hover:bg-red-50 hover:text-red-600 transition-colors"
                                                title="Delete">
                                ...
                                @if(auth()->user()->hasRole('admin'))
                                    ...
                                    <button
                                        onclick="openSectionModal({{ $product->id }}, '{{ addslashes($product->name) }}')"
                                        class="inline-flex items-center gap-1 px-2 py-1.5 text-[10px] font-semibold text-purple-700 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 hover:border-purple-400 transition-colors"
                                        title="Assign to Homepage Sections">
