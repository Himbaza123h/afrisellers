                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <div class="flex items-center justify-center w-8 h-8 bg-purple-100 to-purple-200 rounded-lg">
                        ...
                        <td class="px-4 py-3">
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                @if($profile->verification_status === 'rejected' && $profile->reason_reply)
                                    <button onclick="openReplyModal('{{ addslashes($profile->business_name) }}', '{{ addslashes($profile->rejection_reason) }}', '{{ addslashes($profile->reason_reply) }}')"
                                        class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold text-blue-700 bg-blue-50 rounded-full hover:bg-blue-100 transition">
                                        <i class="fas fa-reply text-[9px]"></i> View Reply
                                    </button>
                                @elseif($profile->verification_status === 'rejected')
                                    <span class="text-[10px] text-gray-400 italic">No reply yet</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button onclick="openNotifyModal({{ $profile->user_id }}, '{{ addslashes($profile->business_name) }}')"
                                    class="p-1.5 text-gray-600 rounded-lg hover:bg-blue-50 hover:text-blue-600" title="Notify Vendor">
                                    <i class="fas fa-bell text-sm"></i>
                                </button>
                               @if($profile->vendor)
                                <a href="{{ route('admin.vendors.edit', $profile->vendor) }}"
                                   class="p-1.5 rounded text-gray-600 hover:bg-gray-100 transition-colors"
                                   title="Edit Vendor">
                                    <i class="fas fa-edit text-xs"></i>
                                </a>
                                @else
                                <span class="p-1.5 rounded text-gray-300 cursor-not-allowed" title="No vendor account yet">
                                    <i class="fas fa-edit text-xs"></i>
                                </span>
                                @endif
                                @if($profile->verification_status === 'pending')
                                    ...
                                    <button type="button" onclick="openRejectModal({{ $profile->id }})"
                                        class="p-1.5 text-gray-600 rounded-lg hover:bg-red-50 hover:text-red-600" title="Reject">
...
function openReplyModal(name, reason, reply) {
    document.getElementById('replyModalName').textContent = name;
    document.getElementById('replyModalReason').textContent = reason || 'No reason provided.';
    document.getElementById('replyModalReply').textContent = reply;
    document.getElementById('replyViewModal').classList.remove('hidden');
}
function closeReplyModal() {
    document.getElementById('replyViewModal').classList.add('hidden');
}
</script>

<!-- Reply View Modal -->
<div id="replyViewModal" class="fixed inset-0 z-50 hidden bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-base font-bold text-gray-900">Vendor Reply</h3>
            <button onclick="closeReplyModal()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Business</p>
        <p class="text-sm font-semibold text-gray-900 mb-4" id="replyModalName"></p>
        ...
        <div class="p-3 bg-red-50 border border-red-200 rounded-lg mb-3">
            ...
            <p class="text-xs text-red-600" id="replyModalReason"></p>
        </div>
        <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p class="text-xs font-semibold text-blue-700 mb-1"><i class="fas fa-reply mr-1"></i> Vendor's Reply:</p>
            <p class="text-xs text-gray-700" id="replyModalReply"></p>
        </div>
        <button onclick="closeReplyModal()"
            class="mt-4 w-full py-2 text-sm font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition">
            Close
        </button>
    </div>
</div>
