...
<div class="mb-4 sm:mb-6 lg:mb-8">
    <div class="flex items-center gap-3 mb-3">
        ...
        <div>
            <h1 class="text-xl sm:text-2xl lg:text-lg font-black text-gray-900 uppercase">Edit Product Category</h1>
            <p class="text-xs sm:text-sm text-gray-600 mt-1">Update product category information</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
    <form action="{{ route('admin.product-category.update', $productCategory) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="border border-gray-300 rounded-lg p-4 max-h-64 overflow-y-auto bg-white">
                @php
                    $selectedCountryIds = old('country_ids', $productCategory->countries->pluck('id')->toArray());
                @endphp
                @forelse($countries as $country)
                    <div class="flex items-center mb-3 last:mb-0">
                        <input
                            ...
                            value="{{ $country->id }}"
                            {{ in_array($country->id, $selectedCountryIds) ? 'checked' : '' }}
