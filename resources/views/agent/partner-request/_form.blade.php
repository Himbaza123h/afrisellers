<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    <div class="p-5 border-b border-gray-100">
        ...
        @if(!$isEdit)
            <p class="text-xs text-gray-500 mt-0.5">
                Use a <strong>different email</strong> from your agent account — this will be your partner login credentials.
            </p>
        @endif
    </div>

    <form
        action="{{ $isEdit ? route('agent.partner-request.update') : route('agent.partner-request.store') }}"
        method="POST"
    ...
    >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @if($partnerRequest?->logo)
                        <div class="mb-2">
                            <img id="agent-logo-preview"
                                src="{{ Storage::url($partnerRequest->logo) }}"
                                class="h-14 w-14 object-contain rounded-lg border border-gray-200 bg-gray-50">
                        </div>
                    @else
                        <div class="mb-2 hidden" id="agent-logo-preview-wrapper">
                            <img id="agent-logo-preview" src=""
                                class="h-14 w-14 object-contain rounded-lg border border-gray-200 bg-gray-50">
                        </div>
                    @endif
                    <input type="file" name="logo" accept="image/*"
                        onchange="agentPreviewLogo(this)"
        ...
        {{-- Credentials (new only) --}}
        @if(!$isEdit)
        <div>
            <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                Partner Login Credentials
                <span class="ml-1 text-gray-300 font-normal normal-case">(different from your agent account)</span>
            </p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required
                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                        placeholder="Must differ from your agent account email">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1">
                    ...
                    <div class="relative">
                        <input type="password" name="password" id="agent-pw-field" required
                            oninput="agentCheckStrength(this.value)"
                            class="w-full px-3 py-2 pr-10 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                            placeholder="Min. 8 characters">
                        <button type="button" onclick="agentTogglePassword('agent-pw-field', this)"
                    ...
                    <div class="mt-2 space-y-1">
                        <div class="w-full bg-gray-100 rounded-full h-1.5">
                            <div id="agent-strength-bar" class="h-1.5 rounded-full transition-all duration-300 w-0 bg-red-500"></div>
                        </div>
                        <p id="agent-strength-label" class="text-xs text-gray-400"></p>
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1">
                    ...
                    <div class="relative">
                        <input type="password" name="password_confirmation" id="agent-confirm-field" required
                            oninput="agentCheckMatch()"
                            class="w-full px-3 py-2 pr-10 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                            placeholder="Repeat your password">
                        <button type="button" onclick="agentTogglePassword('agent-confirm-field', this)"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                            <i class="fas fa-eye text-sm"></i>
                        </button>
                    </div>
                    <p id="agent-match-label" class="text-xs mt-1 hidden"></p>
                </div>
            </div>
        </div>
        @else
...
<script>
function agentTogglePassword(fieldId, btn) {
...
function agentCheckStrength(val) {
    const bar   = document.getElementById('agent-strength-bar');
    const label = document.getElementById('agent-strength-label');
    if (!bar) return;
    let score = 0;
    ...
    const levels = [
        { pct: '20%',  color: 'bg-red-500',    text: 'Very weak',  lc: 'text-red-500'    },
        { pct: '40%',  color: 'bg-orange-500',  text: 'Weak',       lc: 'text-orange-500'  },
        { pct: '60%',  color: 'bg-yellow-500',  text: 'Fair',       lc: 'text-yellow-600'  },
        { pct: '80%',  color: 'bg-blue-500',    text: 'Good',       lc: 'text-blue-600'    },
        { pct: '100%', color: 'bg-green-500',   text: 'Strong',     lc: 'text-green-600'   },
    ];
    ...
    label.className = `text-xs ${level.lc}`;
}

function agentCheckMatch() {
    const pw      = document.getElementById('agent-pw-field')?.value;
    const confirm = document.getElementById('agent-confirm-field')?.value;
    const label   = document.getElementById('agent-match-label');
...
function agentPreviewLogo(input) {
    if (!input.files || !input.files[0]) return;
    const reader = new FileReader();
    reader.onload = e => {
        const wrapper = document.getElementById('agent-logo-preview-wrapper');
        const img     = document.getElementById('agent-logo-preview');
