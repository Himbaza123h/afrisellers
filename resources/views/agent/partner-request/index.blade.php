...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    @if($partner)
        ...
        <div class="bg-white rounded-xl border border-green-200 shadow-sm p-6 text-center space-y-4">
            ...
            <button onclick="switchToPartnerAgent()"
    ...
    @elseif($partnerRequest && $partnerRequest->status === 'rejected')
        ...
        @include('agent.partner-request._form', ['partnerRequest' => $partnerRequest, 'isEdit' => true])
    ...
    @else
        {{-- ── NEW REQUEST ────────────────────────────────────── --}}
        @include('agent.partner-request._form', ['partnerRequest' => null, 'isEdit' => false])
    @endif

</div>

<script>
function switchToPartnerAgent() {
    fetch('{{ route("agent.switch-to-partner") }}', {
        method: 'POST',
        headers: {
