                ...
                <div class="space-y-3">
                    @foreach($order->items as $item)
                        @php
                            ...
                            if ($item->product) {
                                // Try primary image first, then first image
                                $primaryImage = $item->product->images->firstWhere('is_primary', true)
                                    ?? $item->product->images->first();

                                if ($primaryImage) {
                                    $raw = $primaryImage->image_url;
                                    $itemImageUrl = str_starts_with($raw, 'http') ? $raw : asset($raw);
                                } elseif ($item->product->image) {
                                    // fallback to any 'image' accessor if it exists
                                    $raw = $item->product->image;
                                    $itemImageUrl = str_starts_with($raw, 'http') ? $raw : asset($raw);
                                }
                            }
                        @endphp
                        ...
                        <div class="flex gap-3 p-3 border border-gray-200 rounded-lg">
                            @if($itemImageUrl)
                                <img src="{{ $itemImageUrl }}"
                                     ...
                                     class="object-cover w-16 h-16 border border-gray-200 rounded-md flex-shrink-0">
                            @else
                                <div class="flex items-center justify-center flex-shrink-0 w-16 h-16 bg-gray-100 border border-gray-200 rounded-md">
                                ...
                                @if($item->product)
                                    <div class="mt-1">
                                        @if($item->product->status === 'active')
                                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-semibold text-green-700 bg-green-100 rounded">
                                                <i class="fas fa-circle" style="font-size:6px;"></i> Active
                                            </span>
                                        @elseif($item->product->status === 'inactive')
                                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-semibold text-red-700 bg-red-100 rounded">
