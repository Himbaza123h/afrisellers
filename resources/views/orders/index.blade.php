...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="{{ route('admin.orders.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-[#ff0808] text-white rounded-lg hover:bg-[#e00707] transition-all font-medium shadow-md">
                <i class="fas fa-plus"></i>
                <span>Create Order</span>
            </a>
            @endif
            @if(auth()->user()->hasRole('vendor'))
            <a href="{{ route('vendor.orders.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-[#ff0808] text-white rounded-lg hover:bg-[#e00707] transition-all font-medium shadow-md">
                <i class="fas fa-plus"></i>
                <span>Create Order</span>
            </a>
            @endif
        ...
        <div class="stat-card p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $stats['processing'] ?? 0 }}</p>
                    <div class="mt-3 flex items-center gap-1">
        ...
        <div class="stat-card p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $stats['completed'] ?? 0 }}</p>
                    <div class="mt-3 flex items-center gap-1">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <form method="GET" action="{{ route('vendor.orders.index') }}" class="space-y-4">
            <div class="relative flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by order number, customer name or email..." class="w-full pl-11 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="flex flex-wrap gap-3 items-center">
                ...
                @if(request()->hasAny(['search', 'date_range', 'status', 'payment_status', 'sort_by']))
                    <a href="{{ route('vendor.orders.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-blue-100 to-blue-200 rounded-full">
                                        <span class="text-sm font-semibold text-blue-700">{{ substr($order->buyer->name, 0, 1) }}</span>
                                    </div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('vendor.orders.show', $order) }}" class="p-2 text-gray-600 rounded-lg hover:bg-blue-50 hover:text-blue-600" title="View Details">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <button onclick="printInvoice({{ $order->id }})" class="p-2 text-gray-600 rounded-lg hover:bg-purple-50 hover:text-purple-600" title="Print Invoice">
                                        <i class="fas fa-print"></i>
                                    </button>
                                    @if($order->status === 'pending')
                                    <form action="{{ route('vendor.orders.update-status', $order) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="status" value="processing">
                                        <button type="submit" class="p-2 text-gray-600 rounded-lg hover:bg-green-50 hover:text-green-600" title="Process Order">
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm text-gray-500 mb-6">Orders from customers will appear here</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
function printInvoice(orderId) {
    window.open(`/vendor/orders/${orderId}/invoice`, '_blank');
}
