...
<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Orders Management</h1>
            <p class="mt-1 text-xs text-gray-500">Track and manage all customer orders</p>
        </div>
...
@if(auth()->user()->hasRole('admin'))
    <a href="{{ route('admin.orders.create') }}" class="inline-flex items-center gap-2 px-3 py-2 bg-[#ff0808] text-white rounded-lg hover:bg-red-700 transition-all font-medium shadow-sm text-sm">
        <i class="fas fa-plus"></i>
        <span>Create Order</span>
    </a>
@elseif(auth()->user()->hasRole('vendor'))
    <a href="{{ route('vendor.orders.create') }}" class="inline-flex items-center gap-2 px-3 py-2 bg-[#ff0808] text-white rounded-lg hover:bg-red-700 transition-all font-medium shadow-sm text-sm">
        <i class="fas fa-plus"></i>
        <span>Create Order</span>
    </a>
@endif
...
function printReport() {
    window.open('{{ route("admin.orders.print") }}', '_blank');
}
