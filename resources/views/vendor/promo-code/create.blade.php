...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Create Promo Code</h1>
            <p class="mt-1 text-xs text-gray-500">Set up a new promotional discount code</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="number" name="discount_value" id="discountValue" value="{{ old('discount_value') }}" required min="0" step="0.01"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 text-gray-900"
                        placeholder="e.g., 20">
                    <p id="discountValueHint" class="mt-1 text-xs text-gray-500">Maximum 40% for percentage discounts.</p>
                </div>
...
<script>
    ...
    function toggleMaxDiscount() {
        ...
        if (discountType === 'percentage') {
            ...
            discountValue.setAttribute('max', '40');
            discountValueHint.textContent = 'Maximum 40% for percentage discounts.';
            discountValueHint.classList.remove('hidden');
            // Clamp current value if already over 40
            if (parseFloat(discountValue.value) > 40) {
                discountValue.value = 40;
            }
        } else {
            maxDiscountField.style.display = 'none';
            maxDiscountField.querySelector('input').value = '';
            discountValue.removeAttribute('max');
            discountValueHint.textContent = '';
        }
    }
    ...
    document.getElementById('discountValue').addEventListener('input', function() {
        const discountType = document.getElementById('discountType').value;
        const errorId = 'percentageError';
        let existing = document.getElementById(errorId);

        if (discountType === 'percentage' && parseFloat(this.value) > 40) {
            this.value = 40;
            if (!existing) {
                const msg = document.createElement('p');
                msg.id = errorId;
                msg.className = 'mt-1 text-xs text-red-600 font-semibold';
                msg.textContent = 'Percentage discount cannot exceed 40%.';
