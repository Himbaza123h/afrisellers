...
<div class="space-y-6">
    ...
    <form action="{{ route('vendor.product.store') }}" method="POST" class="space-y-6" id="productForm" enctype="multipart/form-data">
        ...
        <!-- Basic Information -->
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6" style="min-height: 1000px !important; height: auto !important;">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div id="variationsContainer" class="space-y-4"></div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div id="specificationsContainer" class="space-y-4"></div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <h2 class="text-lg font-bold text-gray-900 mb-2 flex items-center gap-2">
                ...
                Product Images
                <span id="imgCounter" class="ml-auto text-sm font-normal text-gray-500">0 / 4</span>
            </h2>
            <p class="text-xs text-gray-500 mb-5">Up to 4 images total. JPG, PNG, WebP — max 5 MB each.</p>
            ...
            <div id="imageUploadContainer" class="space-y-4">
                ...
                <div id="imgDropZone"
                    ...
                    <p class="text-sm text-gray-500 font-medium">Click or drag images here</p>
                    <p class="text-xs text-gray-400 mt-1">JPG, PNG, WebP — max 5 MB each — up to 4 images</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="min_order_quantity" id="min_order_quantity"
                        value="{{ old('min_order_quantity', 1) }}" min="1"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900"
                        placeholder="1">
...
{{-- Quill JS must load BEFORE @push scripts --}}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── ONE Quill instance — Description only ────────────────────────
    ...
    const savedDesc = document.getElementById('descriptionInput').value.trim();
    if (savedDesc) descriptionQuill.root.innerHTML = savedDesc;
...
function addImages(files) {
    ...
    incoming.slice(0, available).forEach(f => imageFiles.push(f));
    rebuildImageInput();
    renderImagePreviews();
    updateImgCounter();
    updateDropZone();
...
function renderImagePreviews() {
    ...
    container.innerHTML = '';
    container.classList.toggle('hidden', imageFiles.length === 0);
    imageCount = 0;
            ...
            div.innerHTML = `
                ...
                <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-3">
                    <label class="flex items-center gap-2 text-sm mb-2">
                        <input type="checkbox" name="image_is_primary[${imageCount}]" value="1"
                            ${index === 0 ? 'checked' : ''} class="rounded">
...
function updateImgCounter() {
    ...
    if (el) el.textContent = `${imageFiles.length} / 4`;
}
