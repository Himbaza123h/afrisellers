                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-5">
                                <div>
                                    ...
                                    <input type="text" name="variations[{{ $index }}][variation_type]" required
                                        class="px-3 py-2 w-full text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500" value="{{ $variation->variation_type }}">
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="variations[{{ $index }}][variation_name]" required
                                        class="px-3 py-2 w-full text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500" value="{{ $variation->variation_name }}">
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="variations[{{ $index }}][variation_value]"
                                        class="px-3 py-2 w-full text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500" value="{{ $variation->variation_value }}">
                                </div>
                                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4" id="existingImagesContainer">
                        @foreach($product->images->sortBy('sort_order') as $index => $image)
                            <div class="relative rounded-xl overflow-hidden border border-gray-200 group existing-image-item" data-image-id="{{ $image->id }}">
                                ...
                                @endif
                                <button type="button" onclick="removeExistingImage({{ $image->id }})" class="absolute top-2 right-2 bg-red-600 text-white w-7 h-7 rounded-lg hover:bg-red-700 shadow-lg">
...
<script>
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    reader.onload = function(e) {
                        ...
                        previewDiv.innerHTML = `
                            ...
                            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-3">
                                <label class="flex items-center gap-2 text-sm mb-2">
                                    <input type="checkbox" name="image_is_primary[${imageCount}]" value="1" class="new-primary-checkbox rounded" onchange="updatePrimaryStatusNew(this)">
                                    <span class="text-xs text-white font-medium">Primary Image</span>
                                </label>
                                <input type="hidden" name="image_sort_order[${imageCount}]" value="${existingImageCount + imageCount}">
    ...
    function removeExistingImage(imageId) {
        ...
        if (imageItem) {
            const hiddenInput = imageItem.querySelector('.existing-image-id');
            if (hiddenInput) {
                hiddenInput.remove();
            }
            imageItem.remove();
    ...
    function updatePrimaryStatus(imageId) {
        document.querySelectorAll('.existing-primary-checkbox').forEach(cb => {
            if (cb.closest('[data-image-id]').dataset.imageId != imageId) {
                cb.checked = false;
            }
        });
        document.querySelectorAll('.new-primary-checkbox').forEach(cb => {
            cb.checked = false;
        });
    }
