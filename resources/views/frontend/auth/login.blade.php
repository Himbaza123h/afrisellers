...
<!-- Vendor Rejected Panel -->
@if(session('vendor_rejected'))
    <div id="rejectionPanel" class="bg-red-50 border border-red-300 rounded-xl p-5 space-y-4">
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-red-600 mt-0.5">Hi {{ session('user_name') }}, your business <span class="font-semibold">{{ session('business_name') }}</span> was not approved.</p>
            </div>
        </div>

        @if(session('rejection_reason'))
            <div class="bg-white border border-red-200 rounded-lg p-3">
                <p class="text-xs font-semibold text-gray-700 mb-1">Reason from admin:</p>
                <p class="text-xs text-gray-600">{{ session('rejection_reason') }}</p>
            </div>
        @endif
        ...
        @if(session('reply_sent'))
            <div class="bg-green-50 border border-green-200 rounded-lg p-3">
                <p class="text-xs text-green-700 font-medium">
                    <i class="fas fa-check-circle mr-1"></i> Your reply has been sent to the admin.
                </p>
            </div>
        @else
            <form method="POST" action="{{ route('auth.vendor.rejection.reply') }}" id="replyForm">
                @csrf
                <input type="hidden" name="business_profile_id" value="{{ session('business_profile_id') }}">
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Send a reply to the admin:</label>
                <textarea name="reason_reply" rows="3" required
                    placeholder="Explain your situation or ask for clarification..."
                    class="w-full px-3 py-2 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-400 focus:border-red-400 resize-none mb-2"></textarea>
                <button type="submit" id="replySubmitBtn"
                    class="w-full inline-flex justify-center items-center gap-2 py-2 text-xs font-semibold text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors">
                    <i class="fas fa-reply" id="replyIcon"></i>
            ...
            <form method="POST" action="{{ route('auth.signin.submit') }}" class="space-y-4">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        ...
                        <a href="{{ route('password.request') }}" class="text-xs font-medium text-gray-600 hover:text-[#ff0808] transition-colors">
...
// Auto-hide only error/success banners, NOT the rejection panel
setTimeout(() => {
    const errorMessages = document.querySelectorAll('.bg-green-50, .bg-red-50:not(#rejectionPanel)');
    errorMessages.forEach(msg => {
        if (msg.closest('#rejectionPanel')) return;
