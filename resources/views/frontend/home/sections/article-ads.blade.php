{{--
    ╔══════════════════════════════════════════════════════════════╗
    ║        AFRISELLERS — ARTICLE DETAIL AD BANNER                ║
...
@php
// ── Real ads from DB ──────────────────────────────────────────────
$realArticleAds = \App\Models\Advertisement::where('position', 'article_detail')
    ->where('status', 'running')
    ->where('end_date', '>=', now())
    ->orderBy('approved_at', 'desc')
    ->get();

$articleAdsMapped = $realArticleAds->count() > 0
    ? $realArticleAds->map(fn($ad) => [
        'real_id' => $ad->id,
        'type'    => $ad->type,
        'media'   => $ad->media_url,
...
$artAdPairs = array_chunk($articleAdsMapped, 2);
$artAdTotal = count($artAdPairs);
@endphp

{{-- ═══════════════════ ARTICLE DETAIL AD BANNER ═══════════════════ --}}
<div class="relative overflow-hidden bg-[#0a0a0a] mb-3"
     id="article-detail-ad-banner"
     style="height:112px; border-radius:3px;">

    @foreach($artAdPairs as $pairIndex => $pair)
    <div class="art-ad-pair absolute inset-0 flex transition-opacity duration-1000 ease-in-out"
         style="gap:2px; opacity:{{ $pairIndex === 0 ? '1' : '0' }}; z-index:{{ $pairIndex === 0 ? '2' : '1' }};">

        @foreach($pair as $adIndex => $ad)
        <a href="{{ $ad['cta_url'] ?? '#' }}"
           class="relative flex-1 overflow-hidden group art-ad-cell"
           data-real-id="{{ $ad['real_id'] ?? '' }}"
           onclick="trackArtAdClick(this)">
            ...
            {{-- Shimmer --}}
            <div class="art-ad-shimmer absolute inset-0 pointer-events-none"></div>
...
{{-- ═══════════ STYLES ═══════════ --}}
<style>
.art-ad-cell { -webkit-tap-highlight-color:transparent; text-decoration:none !important; display:block; }
.art-ad-shimmer {
    background: linear-gradient(105deg, transparent 35%, rgba(255,255,255,.06) 50%, transparent 65%);
    transform: translateX(-110%);
}
.art-ad-cell:hover .art-ad-shimmer {
    transform: translateX(110%);
...
<script>
(function(){
    const TOTAL    = {{ $artAdTotal }};
    const INTERVAL = 5500;
    let   cur = 0, timer = null;
    const banner = document.getElementById('article-detail-ad-banner');

    function move(){
        document.querySelectorAll('#article-detail-ad-banner .art-ad-pair').forEach((el, i) => {
    ...
    window.trackArtAdClick = function(el){
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const first = document.querySelector('#article-detail-ad-banner .art-ad-pair');
        if(first){
            first.querySelectorAll('.art-ad-cell').forEach(cell => {
