...
{{-- ═══════════════════ AD BANNER (edge-to-edge, no horizontal inset) ═══════════════════ --}}
<div class="relative left-1/2 w-screen max-w-[100vw] -translate-x-1/2">
<div id="afri-ad-banner" class="relative w-full overflow-hidden bg-[#0a0a0a] rounded-none shadow-md" style="height:112px;">

    {{-- All pairs stacked, fade in/out --}}
    @foreach($adPairs as $pairIndex => $pair)
    <div class="flex absolute inset-0 transition-opacity duration-1000 ease-in-out ad-pair"
        ...
        <a href="{{ $ad['cta_url'] ?? '#' }}"
           ...
           onclick="trackAdClick(this)">
            ...
            @elseif($ad['type'] === 'video')
                <video class="object-cover absolute inset-0 w-full h-full"
            ...
            {{-- ── Shimmer ── --}}
            <div class="absolute inset-0 pointer-events-none ad-shimmer"></div>
        ...
        <a href="#" class="relative flex-1 overflow-hidden group bg-[#111]" style="display:block;text-decoration:none;">
            ...
            <div class="flex relative z-10 justify-center items-center h-full">
                <div class="text-center">
                    ...
                    <p class="text-xs sm:text-sm font-black text-white/60">Full width · Premium slot</p>
                </div>
            </div>
        </a>
        @endif
