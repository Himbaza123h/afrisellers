...
<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Country Vendors Management</h1>
            <p class="mt-1 text-xs text-gray-500">Monitor and manage vendors in {{ $country->name }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <button onclick="window.open('{{ route('country.vendors.print') }}' + window.location.search, '_blank')" class="inline-flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all font-medium shadow-sm text-sm">
    ...
    <div id="tab-overview-content" class="tab-content">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-2 flex items-center gap-1">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                <i class="fas fa-store mr-1 text-[8px]"></i> All vendors
                            </span>
                        </div>
                    </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4 mt-4">
            <form method="GET" action="{{ route('country.vendors.index') }}" class="space-y-3">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('country.vendors.index') }}" class="inline-flex items-center gap-1 px-3 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">Registration</th>
                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">Email</th>
                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">Business</th>
                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">Account</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                    ...
                                    <div class="flex items-center gap-3">
                                        @if($vendor->businessProfile && $vendor->businessProfile->logo)
                                            <img src="{{ asset('storage/' . $vendor->businessProfile->logo) }}"
                                                 ...
                                                 class="vendor-logo w-10 h-10 object-cover rounded-md border border-gray-200">
                                        @else
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p class="text-sm font-medium text-gray-900 truncate">
                                                {{ Str::limit($vendor->businessProfile->business_name ?? 'N/A', 30) }}
                                            </p>
                                            <p class="text-xs text-gray-500 mt-0.5">ID: #{{ $vendor->id }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('country.vendors.show', $vendor->id) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium px-2 py-1 rounded hover:bg-blue-50" title="View">
                                        ...
                                        @if($vendor->account_status === 'active')
                                            <form action="{{ route('country.vendors.suspend', $vendor->id) }}" method="POST" class="inline" onsubmit="return confirm('Suspend this vendor?')">
                                                @csrf
    ...
    <div id="tab-verification-content" class="tab-content hidden">
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div class="p-4 bg-green-50 to-white rounded-lg border border-green-100">
                ...
                <div class="p-4 bg-indigo-50 to-white rounded-lg border border-indigo-100">
                ...
                <div class="p-4 bg-yellow-50 to-white rounded-lg border border-yellow-100">
            ...
            @php
                $pendingVendors = \App\Models\Vendor\Vendor::whereHas('businessProfile', function($query) use ($country) {
                    $query->where('country_id', $country->id)->where('is_admin_verified', false);
                })->take(5)->get();
            @endphp
    ...
    <div id="tab-analytics-content" class="tab-content hidden">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div class="space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-700">Active Vendors</span>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-700">Suspended Vendors</span>
                            <span class="text-sm font-bold text-red-700">{{ number_format($stats['suspended']) }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-3">
                            <div class="bg-red-600 h-3 rounded-full transition-all duration-300" style="width: {{ $stats['total'] > 0 ? round(($stats['suspended'] / $stats['total']) * 100) : 0 }}%"></div>
                        </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-700">Inactive Vendors</span>
                            <span class="text-sm font-bold text-gray-700">{{ number_format($stats['total'] - $stats['active'] - $stats['suspended']) }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-3">
                            <div class="bg-gray-600 h-3 rounded-full transition-all duration-300" style="width: {{ $stats['total'] > 0 ? round((($stats['total'] - $stats['active'] - $stats['suspended']) / $stats['total']) * 100) : 0 }}%"></div>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">{{ $stats['total'] > 0 ? round((($stats['total'] - $stats['active'] - $stats['suspended']) / $stats['total']) * 100) : 0 }}% of total</p>
                    </div>
                </div>
            </div>
            ...
            <!-- Verification Status -->
            <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
                <h3 class="text-base font-semibold text-gray-900 mb-4">Verification Status</h3>
                <div class="space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-700">Business Verified</span>
                            <span class="text-sm font-bold text-purple-700">{{ number_format($stats['business_verified']) }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-3">
                            <div class="bg-purple-600 h-3 rounded-full transition-all duration-300" style="width: {{ $stats['verified_percentage'] }}%"></div>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">{{ $stats['verified_percentage'] }}% of total</p>
                    </div>

                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-700">Email Verified</span>
                            <span class="text-sm font-bold text-indigo-700">{{ number_format($stats['email_verified']) }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-3">
                            <div class="bg-indigo-600 h-3 rounded-full transition-all duration-300" style="width: {{ $stats['total'] > 0 ? round(($stats['email_verified'] / $stats['total']) * 100) : 0 }}%"></div>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">{{ $stats['total'] > 0 ? round(($stats['email_verified'] / $stats['total']) * 100) : 0 }}% of total</p>
                    </div>

                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-700">Email Unverified</span>
                            <span class="text-sm font-bold text-yellow-700">{{ number_format($stats['email_unverified']) }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-3">
                            <div class="bg-yellow-600 h-3 rounded-full transition-all duration-300" style="width: {{ $stats['total'] > 0 ? round(($stats['email_unverified'] / $stats['total']) * 100) : 0 }}%"></div>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">{{ $stats['total'] > 0 ? round(($stats['email_unverified'] / $stats['total']) * 100) : 0 }}% of total</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4 lg:col-span-2">
                <h3 class="text-base font-semibold text-gray-900 mb-4">Recent Vendor Registrations</h3>
                <div class="space-y-3">
                    @php
                        $recentVendors = \App\Models\Vendor\Vendor::whereHas('businessProfile', function($query) use ($country) {
                    ...
                    @forelse($recentVendors as $vendor)
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div>
                                    <p class="text-sm font-medium text-gray-900">{{ $vendor->businessProfile->business_name ?? 'N/A' }}</p>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('country.vendors.show', $vendor->id) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium px-2 py-1 rounded hover:bg-blue-50">
                                    <i class="fas fa-eye"></i>
                                </a>
                            </div>
                        </div>
