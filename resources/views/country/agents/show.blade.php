...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between no-print">
        <div>
            ...
            <p class="text-sm text-gray-500 ml-7">{{ $agent->name }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 no-print">
                ...
                <div class="space-y-2">
                    @if($agent->status === 'active')
                        <form action="{{ route('country.agents.suspend', $agent->id) }}" method="POST">
                            @csrf
                            <button type="submit"
                                onclick="return confirm('Are you sure you want to suspend this agent?')"
                    ...
                    @else
                        <form action="{{ route('country.agents.activate', $agent->id) }}" method="POST">
                            @csrf
                            <button type="submit"
