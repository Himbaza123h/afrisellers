...
<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Country Products Management</h1>
            <p class="mt-1 text-xs text-gray-500">Monitor and manage products in {{ $country->name }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <button onclick="window.open('{{ route('country.products.print') }}' + window.location.search, '_blank')" class="inline-flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all font-medium shadow-sm text-sm">
    ...
    <div id="tab-overview-content" class="tab-content">
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4 mt-4">
            <form method="GET" action="{{ route('country.products.index') }}" class="space-y-3">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('country.products.index') }}" class="inline-flex items-center gap-1 px-3 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">Price Range</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p class="text-sm font-medium text-gray-900 truncate">{{ Str::limit($product->name, 30) }}</p>
                                            <p class="text-xs text-gray-500">{{ $product->created_at->format('M d, Y') }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <p class="text-sm font-medium text-gray-900">{{ $product->user->name ?? 'N/A' }}</p>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('country.products.show', $product->id) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium px-2 py-1 rounded hover:bg-blue-50" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if(!$product->is_admin_verified)
                                            <form action="{{ route('country.products.approve', $product->id) }}" method="POST" class="inline">
                                                @csrf
                                            ...
                                            <form action="{{ route('country.products.reject', $product->id) }}" method="POST" class="inline" onsubmit="return confirm('Reject this product?')">
                                                @csrf
                                        ...
                                        @endif
                                        <form action="{{ route('country.products.destroy', $product->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this product?')">
                                            @csrf @method('DELETE')
    ...
    <div id="tab-categories-content" class="tab-content hidden">
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($categories as $category)
                    @php
                        $categoryCount = \App\Models\Product::where('country_id', $country->id)
                            ->where('product_category_id', $category->id)
                            ->count();
                        $colors = ['blue', 'green', 'purple', 'orange', 'red', 'indigo', 'yellow'];
                        $color = $colors[$loop->index % count($colors)];
                    @endphp
                    <div class="p-4 bg-{{ $color }}-50 to-white rounded-lg border border-{{ $color }}-100">
                        <div class="flex items-center gap-3">
    ...
    <div id="tab-verification-content" class="tab-content hidden">
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="p-4 bg-red-50 to-white rounded-lg border border-red-100">
            ...
            <!-- Pending Approvals -->
            @php
                $pendingProducts = \App\Models\Product::where('country_id', $country->id)
                            ...
                            <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg hover:bg-yellow-100">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-gray-500">{{ $product->user->name ?? 'N/A' }}</p>
                                    </div>
                                </div>
                                <div class="flex gap-1">
                                    ...
                                    <form action="{{ route('country.products.reject', $product->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-red-600 hover:text-red-700 text-sm font-medium px-2 py-1 rounded hover:bg-red-50" title="Reject">
                                            <i class="fas fa-times"></i>
                                        </button>
