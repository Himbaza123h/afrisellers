...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
        <div class="flex items-center gap-2 text-xs text-gray-400 mb-1">
            ...
            <span class="text-gray-600 font-semibold">Contact Details</span>
        </div>
        <h1 class="text-lg font-black text-gray-900">Contact Details</h1>
        <p class="text-xs text-gray-500 mt-0.5">Your contact person information</p>
    </div>
    <a href="{{ route('partner.contact.edit') }}"
...
<div class="grid grid-cols-3 gap-3 mb-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-3">
        <div class="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
            <i class="fas fa-address-card text-gray-500 text-sm"></i>
        </div>
        <div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm">
    ...
    <div class="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        {{-- Filter pills --}}
        <div class="flex items-center gap-1.5" id="filter-pills">
    ...
    @php
    $fields = [
        ['field' => 'contact_name',     'icon' => 'fas fa-user',        'color' => 'text-blue-600',   'bg' => 'bg-blue-50',   'label' => 'Full Name'],
        ['field' => 'contact_position', 'icon' => 'fas fa-briefcase',   'color' => 'text-purple-600', 'bg' => 'bg-purple-50', 'label' => 'Position / Role'],
        ['field' => 'email',            'icon' => 'fas fa-envelope',    'color' => 'text-red-500',    'bg' => 'bg-red-50',    'label' => 'Email Address'],
        ['field' => 'phone',            'icon' => 'fas fa-phone',       'color' => 'text-green-600',  'bg' => 'bg-green-50',  'label' => 'Phone Number'],
        ['field' => 'whatsapp',         'icon' => 'fab fa-whatsapp',    'color' => 'text-emerald-600','bg' => 'bg-emerald-50','label' => 'WhatsApp'],
    ];
    @endphp
    ...
    <div id="view-grid" class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
        @foreach($fields as $f)
        @php $val = $partner?->{$f['field']}; @endphp
        <div class="contact-field-item flex items-center gap-3 p-4 rounded-lg border
            ...
            <div class="flex-1 min-w-0">
                ...
                @if($val)
                    <p class="text-xs font-semibold text-gray-900 truncate">{{ $val }}</p>
                @else
    ...
    <div id="view-list" class="hidden divide-y divide-gray-100">
        @foreach($fields as $f)
        @php $val = $partner?->{$f['field']}; @endphp
        <div class="contact-field-item flex items-center gap-4 px-5 py-3.5 {{ $val ? 'hover:bg-gray-50' : 'opacity-60' }} transition-all"
             data-filled="{{ $val ? '1' : '0' }}"
             data-label="{{ strtolower($f['label']) }}">
            <div class="w-8 h-8 {{ $f['bg'] }} rounded-lg flex items-center justify-center flex-shrink-0">
                <i class="{{ $f['icon'] }} {{ $f['color'] }} text-sm"></i>
            </div>
            <p class="text-sm font-semibold text-gray-700 w-36 flex-shrink-0">{{ $f['label'] }}</p>
            <div class="flex-1 min-w-0">
                @if($val)
                    <p class="text-xs font-semibold text-gray-900 truncate">{{ $val }}</p>
                @else
...
<div class="mt-4 bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-center justify-between gap-3">
    <div class="flex items-center gap-3">
        ...
        <p class="text-xs text-amber-700 font-semibold">
            You have {{ $stats['missing'] }} field{{ $stats['missing'] > 1 ? 's' : '' }} not yet filled. Complete your contact profile.
        </p>
    </div>
    <a href="{{ route('partner.contact.edit') }}"
...
{{-- ── JS ───────────────────────────────────────────────────── --}}
<script>
const CONTACT_LAYOUT_KEY = 'contact_layout';
...
function setLayout(mode) {
    ...
    localStorage.setItem(CONTACT_LAYOUT_KEY, mode);
}
...
function setFilter(mode) {
    ...
    document.querySelectorAll('.filter-pill').forEach(btn => {
        const isActive = btn.textContent.trim().toLowerCase() === mode
                      || (mode === 'all' && btn.textContent.trim() === 'All');
...
// ── Search ─────────────────────────────────────────────────
function filterFields() { applyFilters(); }

function applyFilters() {
    const query = (document.getElementById('contact-search')?.value || '').toLowerCase().trim();
    const items = document.querySelectorAll('.contact-field-item');
    let visible = 0;
...
document.addEventListener('DOMContentLoaded', () => {
    const saved = localStorage.getItem(CONTACT_LAYOUT_KEY) || 'grid';
    setLayout(saved);
    setFilter('all');
});
</script>
