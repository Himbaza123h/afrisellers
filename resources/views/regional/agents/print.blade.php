...
<head>
    ...
    <title>Agents Report – {{ $region->name }}</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .country-box .c-name { font-size: 11px; font-weight: 600; color: #3730a3; }
        .country-box .c-count { font-size: 16px; font-weight: 700; color: #1e1b4b; margin-top: 2px; }
        .country-box .c-active { font-size: 10px; color: #6b7280; }
...
<div class="page">
    ...
    <div class="header">
        <div class="header-left">
            <h1>Regional Agents Report</h1>
            <p>{{ $region->name }} &bull; Regional Admin Panel</p>
        </div>
    ...
    {{-- Country Breakdown --}}
    @if(count($stats['per_country']) > 0)
        <p class="section-title">Breakdown by Country</p>
        <div class="countries-grid" style="margin-bottom: 16px;">
            @foreach($stats['per_country'] as $row)
                <div class="country-box">
                    <div class="c-name">{{ $row['country']->name }}</div>
                    <div class="c-count">{{ number_format($row['total']) }}</div>
                    <div class="c-active">{{ $row['active'] }} active</div>
                </div>
            @endforeach
        </div>
    @endif
    ...
    @php
        $filters = array_filter([
            ...
            request('country_id') ? 'Country ID: '.request('country_id') : null,
    ...
    @else
        <div class="filter-bar">
            Showing all <span>{{ $agents->count() }}</span> agents across
            <span>{{ $countries->count() }}</span> {{ Str::plural('country', $countries->count()) }}
            in {{ $region->name }}
        </div>
    @endif
    ...
    {{-- Footer --}}
    <div class="footer">
        <span>Afrisellers &bull; {{ $region->name }} &bull; Regional Agents Report</span>
