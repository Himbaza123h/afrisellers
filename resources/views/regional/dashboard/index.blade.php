...
<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">{{ $region->name }} Regional Dashboard</h1>
            <p class="text-xs text-gray-600">Welcome back, {{ auth()->user()->name }}!</p>
        </div>

        <!-- Filters -->
        <form method="GET" action="{{ route('regional.dashboard.home') }}" class="flex gap-2">
            <select name="period" onchange="this.form.submit()" class="px-3 py-2 text-sm rounded-lg border-gray-300 focus:border-[#ff0808] focus:ring-[#ff0808]">
            ...
            <select name="country_id" onchange="this.form.submit()" class="px-3 py-2 text-sm rounded-lg border-gray-300 focus:border-[#ff0808] focus:ring-[#ff0808]">
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $stats['total_countries'] }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <!-- Vendor Growth Chart -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Vendor Growth</h3>
            <canvas id="vendorGrowthChart"></canvas>
        </div>

        <!-- Product Growth Chart -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Product Growth</h3>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <!-- Revenue Chart -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Revenue Trend</h3>
            <canvas id="revenueChart"></canvas>
        </div>

        <!-- Country Distribution -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Vendors by Country</h3>
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <h3 class="text-base font-semibold text-gray-900 mb-3">Country Performance</h3>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Country</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Vendors</th>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <!-- Top Categories -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Top Product Categories</h3>
        ...
        <!-- Pending Approvals -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Pending Vendor Approvals</h3>
            @if($pendingApprovals->count() > 0)
                <div class="space-y-2">
                    @foreach($pendingApprovals as $profile)
                        <div class="flex items-center justify-between p-2 bg-orange-50 rounded">
                            <div>
                                <p class="text-sm font-medium text-gray-900">{{ Str::limit($profile->business_name, 25) }}</p>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <!-- Recent Vendors -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Recent Vendors</h3>
            @if($recentVendors->count() > 0)
                <div class="space-y-2">
                    @foreach($recentVendors as $vendor)
                        <div class="flex items-center justify-between p-2 bg-gray-50 rounded">
                            <div>
                                <p class="text-sm font-medium text-gray-900">{{ Str::limit($vendor->businessProfile->business_name, 25) }}</p>
        ...
        <!-- Recent Products -->
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            <h3 class="text-base font-semibold text-gray-900 mb-3">Recent Products</h3>
