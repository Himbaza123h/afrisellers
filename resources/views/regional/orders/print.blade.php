...
<head>
    <meta charset="UTF-8">
    <title>Regional Orders Report - {{ $region->name }} - {{ now()->format('M d, Y') }}</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 15mm;
        ...
        .badge-shipped { background-color: #c7d2fe; color: #3730a3; }
        ...
        .badge-refunded { background-color: #e9d5ff; color: #6b21a8; }
...
<body>
    <!-- Header -->
    <div class="header">
        <h1>REGIONAL ORDERS REPORT - {{ strtoupper($region->name) }}</h1>
            ...
            @forelse($orders as $order)
                <tr>
                    ...
                    <td>
                        ...
                        <br><small>{{ Str::limit($order->buyer->email ?? 'N/A', 20) }}</small>
                    </td>
                    <td>
                        <strong>{{ $order->vendor->name ?? 'N/A' }}</strong>
                        <br><small>{{ Str::limit($order->vendor->email ?? 'N/A', 20) }}</small>
                    </td>
                    <td class="text-center">{{ number_format($order->items->count()) }}</td>
                    ...
                    <td>
                        ...
                        @endphp
                        <span class="badge {{ $paymentStatusClass }}">{{ ucfirst($order->payment_status ?? 'pending') }}</span>
                    </td>
                    <td>
                        @php
                    ...
                    <td>
                        @php
                            $vendorBusinessProfile = $order->vendor->businessProfile ?? null;
                            $countryName = $vendorBusinessProfile && $vendorBusinessProfile->country
                                ? $vendorBusinessProfile->country->name
                                : 'N/A';
                        @endphp
                        {{ $countryName }}
                    </td>
    ...
    <table class="main-table">
        <tbody>
            ...
            <tr>
                <td style="font-weight: bold;">Order Status Distribution</td>
                <td>
            ...
            <tr>
                <td style="font-weight: bold;">Revenue Summary</td>
                <td>
            ...
            <tr>
                <td style="font-weight: bold;">Cancelled Orders</td>
                <td>{{ $stats['cancelled'] }} orders</td>
            </tr>
        </tbody>
    </table>
    ...
    <!-- Footer -->
    <div class="footer">
        <p>Regional Admin Order Management System</p>
        <p>Page 1 of 1 | Report ID: RO-{{ now()->format('Ymd-His') }} | Region: {{ $region->name }}</p>
    </div>
