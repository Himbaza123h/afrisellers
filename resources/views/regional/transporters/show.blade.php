    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between no-print">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('regional.transporters.index') }}" class="text-gray-600 hover:text-gray-900 transition-colors">
        ...
        <div class="flex flex-wrap gap-3">
            @if($transporter->verification_status == 'pending')
                <form action="{{ route('regional.transporters.verify', $transporter->id) }}" method="POST" class="inline">
                    @csrf
