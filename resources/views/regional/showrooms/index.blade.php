...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Regional Showrooms Management</h1>
            <p class="mt-1 text-sm text-gray-500">Monitor and manage showrooms across {{ $region->name }} region ({{ $countries->count() }} countries)</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="stat-card p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-sm font-medium text-gray-600 mb-1">Active Showrooms</p>
        ...
        <div class="stat-card p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-sm font-medium text-gray-600 mb-1">Verified Showrooms</p>
        ...
        <div class="stat-card p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['featured']) }}</p>
                    <div class="mt-3 flex items-center gap-1">
        ...
        <div class="stat-card p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_inquiries']) }}</p>
                    <div class="mt-3 flex items-center gap-1">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
                            <i class="fas fa-comments mr-1 text-[10px]"></i> All time
                        </span>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <form method="GET" action="{{ route('regional.showrooms.index') }}" class="space-y-4">
            <div class="relative flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search showrooms by name, city, or owner..." class="w-full pl-11 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="flex flex-wrap gap-3 items-center">
                ...
                <select name="city" class="pl-4 pr-10 py-2.5 border border-gray-300 rounded-lg appearance-none bg-white">
                ...
                @if(request()->hasAny(['search', 'country_id', 'city', 'date_range', 'status', 'verification', 'featured', 'business_type', 'sort_by']))
                    <a href="{{ route('regional.showrooms.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-left text-gray-700 uppercase">Country/City</th>
                        ...
                        <tr class="hover:bg-gray-50">
                                ...
                                <div class="flex items-center gap-3">
                                    @if($showroom->primary_image)
                                        <img src="{{ asset('storage/' . $showroom->primary_image) }}"
                                             alt="{{ $showroom->name }}"
                                             class="showroom-image w-16 h-16 object-cover rounded-lg border border-gray-200">
                                    @else
                                    ...
                                    <div class="flex flex-col">
                                        <div class="flex items-center gap-2">
                                            <span class="text-sm font-semibold text-gray-900">{{ Str::limit($showroom->name, 25) }}</span>
                                        ...
                                        <span class="text-xs text-gray-500">{{ $showroom->created_at->format('M d, Y') }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col gap-1">
                                    <span class="px-2.5 py-1 rounded-md text-xs font-medium bg-blue-100 text-blue-800 inline-block w-fit">
                                        {{ $showroom->country->name ?? 'N/A' }}
                                    </span>
                                    <span class="text-sm text-gray-900">{{ $showroom->city }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm font-medium text-gray-900">{{ $showroom->user->name ?? 'N/A' }}</span>
                                    <span class="text-xs text-gray-500">{{ $showroom->user->email ?? 'N/A' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('regional.showrooms.show', $showroom->id) }}" class="p-2 text-gray-600 rounded-lg hover:bg-blue-50 hover:text-blue-600" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    @if(!$showroom->is_verified)
                                        <form action="{{ route('regional.showrooms.verify', $showroom->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="p-2 text-gray-600 rounded-lg hover:bg-green-50 hover:text-green-600" title="Verify">
                                    ...
                                    <form action="{{ route('regional.showrooms.feature', $showroom->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="p-2 text-gray-600 rounded-lg hover:bg-yellow-50 hover:text-yellow-600" title="{{ $showroom->is_featured ? 'Unfeature' : 'Feature' }}">
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-semibold text-gray-900 mb-1">No showrooms found</p>
                                    <p class="text-sm text-gray-500">Showrooms from your region will appear here</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
