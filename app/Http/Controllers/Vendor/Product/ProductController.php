...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($currentSubscription) {
            $maxProducts = $currentSubscription->plan->getFeature('max_products');
            if ($maxProducts !== null) {
                $currentCount = Product::where('user_id', auth()->id())->count();
                if ($currentCount >= (int) $maxProducts) {
                    return back()->withInput()->withErrors([
                        'error' => 'You have reached your plan limit of ' . $maxProducts . ' products. Please upgrade your plan.'
                    ]);
                }
            }
        }

        Log::info($maxProducts);
        ...
        $validated['is_admin_verified'] = true;
