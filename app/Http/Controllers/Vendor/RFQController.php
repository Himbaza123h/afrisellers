...
class RFQController extends Controller
{
    ...
    {
        $vendor = Vendor::with(['businessProfile'])->where('user_id', auth()->id())->first();
...
$currentSubscription = \App\Models\Subscription::where('seller_id', $vendor->user_id)
    ->where('status', 'active')
    ->with('plan.features')
    ->first();

$activeTrial = \App\Models\VendorTrial::where('user_id', $vendor->user_id)
...
if ($activeTrial) {
    ...
    $rfqFeature = $currentSubscription->plan->features
        ->where('feature_key', 'has_rfq_access')
        ->first();
    $hasRfqAccess = $rfqFeature && in_array(strtolower($rfqFeature->feature_value), ['true', '1', 'yes']);
}

if (!$hasRfqAccess) {
    return redirect()->route('vendor.dashboard.home')
        ->with('error', 'Your current plan does not include RFQ access. Please upgrade your plan.');
}

        // Get the RFQ limit from subscription plan (default to 50 if not set)
        $rfqLimit = $currentSubscription?->plan?->buyer_rfqs_limit ?? 50;
...
if (!$hasRfqAccess) {
    return redirect()->route('vendor.dashboard.home')
        ...
        $rfqLimit = $currentSubscription?->plan?->buyer_rfqs_limit ?? 50;
            ...
            // Mark the rfq as read for the vendor
            $rfq->tracking = 'read';
            $rfq->save();
