...
private function getStats($request = null)
{
    ...
    public function purchase(Request $request, $addonId)
    {
        ...
        // Get business profile (supplier_id FK → business_profiles.id, not vendors.id)
        $businessProfile = BusinessProfile::where('user_id', $user->id)->first();

        if (! $businessProfile) {
            return back()->with('error', 'You need a business profile before purchasing add-ons.');
        }
        ...
        $relatedId = $request->type === 'supplier'
            ? $businessProfile->id
            : $request->related_id;

        if ($request->type !== 'supplier' && $relatedId === null) {
            return back()->with('error', 'Please select an item to promote.');
        }
        ...
        $existingAddon = AddonUser::where('user_id', $user->id)
            ...
            ->where($request->type.'_id', $relatedId)
            ->active()
            ->first();
    ...
    try {
        ...
        return back()->with('error', 'Failed to repurchase addon: ' . $e->getMessage());
    }
    }
    ...
    public function cancel($id)
    {
       ...
       return redirect()->route('vendor.addons.index')
    ->with('success', 'Addon cancelled successfully!');
