...
class AgentPartnerRequestController extends Controller
{
    public function index()
    {
        $user = auth()->user();

        $partnerRequest = PartnerRequest::where('agent_user_id', $user->id)
            ->latest()
            ->first();
        ...
        return view('agent.partner-request.index', compact('partnerRequest', 'partner'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $existing = PartnerRequest::where('agent_user_id', $user->id)->first();
        if ($existing) {
        ...
        // 1. Create User with partner credentials (different email from agent account)
        ...
        // 2. Create PartnerRequest linked to partner user + agent user
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('logo')) {
            if ($partnerRequest->logo) {
                Storage::disk('public')->delete($partnerRequest->logo);
            }
    ...
    public function switchToPartner()
    {
        ...
        $partnerRequest = PartnerRequest::where('agent_user_id', $user->id)->first();
