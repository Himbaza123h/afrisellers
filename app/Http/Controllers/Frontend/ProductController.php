...
class ProductController extends Controller
{
        ...
        public function trackClick(Product $product, Request $request)
    {
        ...
        $performance = Performance::firstOrCreate(
            ...
            ProductAnalytics::alltime($product->id)->increment('clicks');
    ProductAnalytics::today($product->id)->increment('clicks');
        ...
        public function trackImpression(Product $product, Request $request)
        {
            try {
                ...
                ProductAnalytics::alltime($product->id)->increment('impressions');
...
public function show($slug, Request $request)
{
    ...
    ProductAnalytics::alltime($product->id)->increment('views');
    ProductAnalytics::today($product->id)->increment('views');
    ...
    public function categoryProducts(string $slug, Request $request)
    {
        ...
        $category = \App\Models\ProductCategory::where('status', 'active')
            ->get()
            ->first(function ($cat) use ($slug) {
                return \Illuminate\Support\Str::slug($cat->name) === $slug
                    || $cat->slug === $slug;
        ...
        // ── Products query ────────────────────────────────────────────────
        $query = \App\Models\Product::where('product_category_id', $category->id)
            ->where('status', 'active')
            ->where('is_admin_verified', true)
            ->with([
                'images',
        ...
        // Apply shared filters (reuse existing private method)
        $query = $this->applyFilters($query, $request);
        $query = $this->applySorting($query, $request->get('sort', 'latest'));

        $products = $query->paginate(24)->withQueryString();

        // ── Countries available in this category (for the filter sidebar) ─
        $countryIds = \App\Models\Product::where('product_category_id', $category->id)
            ->where('status', 'active')
            ->where('is_admin_verified', true)
        ...
        $availableCountries = \App\Models\Country::whereIn('id', $countryIds)
            ->orderBy('name')
            ->get();

        // ── Sibling categories (for the "Other Categories" footer row) ────
        $relatedCategories = \App\Models\ProductCategory::where('status', 'active')
