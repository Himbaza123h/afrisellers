...
class BuyerController extends Controller
{
    ...
    public function uploadExtraDocument(Request $request)
{
    $request->validate([
    ...
    // Delete old extra document if exists
    if ($profile->extra_document && Storage::disk('public')->exists($profile->extra_document)) {
        Storage::disk('public')->delete($profile->extra_document);
    }

    $file = $request->file('extra_document');
    $path = $file->store('business-profiles/extra-documents', 'public');

    $profile->update([
        'extra_document'               => $path,
        'extra_document_original_name' => $file->getClientOriginalName(),
        'extra_document_uploaded_at'   => now(),
        // Reset status so admin sees it as pending again
        'verification_status'          => 'pending',
        'rejection_reason'             => null,
    ]);

    return back()->with('success', 'Document submitted successfully. Your application is under review again.');
}
   ...
   public function showBecomeVendor()
{
    ...
    if ($existingVendor?->owner_id_document_id) {
        return redirect()->route('vendor.dashboard.home')->with('info', 'You already have a vendor account.');
    }
    if ($existingVendor && ! $existingVendor->owner_id_document_id) {
        return redirect()->route('buyer.become-vendor.step2', [
            'id' => $existingVendor->business_profile_id,
        ])->with('info', 'Please upload your documents to complete vendor registration.');
    }

    // Check if user has already submitted a business profile request
    $existingProfile = BusinessProfile::where('user_id', $user->id)->latest()->first();
    if ($existingProfile) {
        $countries = Country::where('status', 'active')->orderBy('name')->get();
        return view('buyer.become-vendor', compact('existingProfile', 'countries'));
    }
...
public function storeBecomeVendor(Request $request)
{
    ...
    try {
        ...
        $defaultPlan = Plan::where('is_default', true)->first()
            ?? Plan::orderBy('id', 'asc')->first();

        // Create vendor account automatically
        $verificationToken = str_pad(random_int(0, 999999), 6, '0', STR_PAD_LEFT);
        ...
        // Link vendor back to business profile
        ...
        $trialAlreadyExists = VendorTrial::where('user_id', $user->id)->exists();
    ...
    public function showBecomeVendorStep2($id)
    {
        ...
        if ($existingVendor && (int) $existingVendor->business_profile_id !== (int) $businessProfile->id) {
            return redirect()->route('buyer.become-vendor.step2', [
                'id' => $existingVendor->business_profile_id,
            ]);
        }
    ...
    public function storeBecomeVendorStep2(Request $request, $id)
    {
        ...
        try {
            ...
            $ownerID = OwnerID::updateOrCreate(
                ['user_id' => $user->id],
                [
            ...
            if ($existingVendor) {
                $existingVendor->update(['owner_id_document_id' => $ownerID->id]);
            }
    ...
    public function showBecomeAgent()
{
    $user     = auth()->user();
    $requests = AgentRequest::where('user_id', $user->id)->latest()->get();
    $hasPending   = $requests->where('status', 'pending')->count() > 0;
    $countries    = Country::orderBy('name')->get();

    return view('buyer.become-agent', compact('requests', 'hasPending', 'countries'));
}
...
public function storeBecomeAgent(Request $request)
{
    ...
    $existing = AgentRequest::where('user_id', $user->id)
        ->where('status', 'pending')
        ->first();

    if ($existing) {
        return back()->with('error', 'You already have a pending agent request.');
    }
    ...
    return back()->with('success', 'Your agent request has been submitted. We will review it and notify you by email.');
}
}
