...
public function toggleArticleLike(Request $request, $businessProfileId, $articleSlug)
{
    $article = \App\Models\Article::where('slug', $articleSlug)->firstOrFail();
    $ip = $request->ip();

    $existing = \App\Models\ArticleLike::where('article_id', $article->id)
                                        ->where('ip_address', $ip)
                                        ->first();
    ...
    // Sync real count
    $article->update(['likes_count' => \App\Models\ArticleLike::where('article_id', $article->id)->count()]);

    if ($request->expectsJson()) {
        return response()->json([
            'liked'  => $liked,
            'count'  => $article->fresh()->likes_count,
        ]);
    }
...
public function showBusinessProfile($id, $name = null)
{
    ...
    ProfileAnalytics::alltime($businessProfile->id)->increment('views');
ProfileAnalytics::today($businessProfile->id)->increment('views');
...
private function getFeaturedBusinessesWithProducts($limit = 3, $onlyWithActiveAddons = false)
{
    ...
    // Get all published articles for this business
    $articles = Article::where('user_id', $businessProfile->user_id)
        ->published()
        ->with(['user'])
        ->latest('published_at')
        ->paginate(9);
        ...
        return view('frontend.business-profile.products', compact('businessProfile', 'articles', 'products', 'categories'));
    }


public function singleArticle($businessProfileId, $articleSlug, Request $request){
    $businessProfile = BusinessProfile::with(['user', 'country'])
        ->findOrFail($businessProfileId);
    ...
    // Fetch the article by slug for this user
    $article = Article::where('slug', $articleSlug)
        ->where('user_id', $businessProfile->user_id)
        ->where('status', 'published')
        ->with(['user', 'comments.replies'])
        ->firstOrFail();
    ...
    $relatedArticles = Article::related($article)
        ->limit(6)
        ->get();

    return view('frontend.business-profile.singleArticle', compact('businessProfile', 'article', 'relatedArticles'));
}

public function storeArticleComment(Request $request, $businessProfileId, $articleSlug)
{
    $request->validate([
    ...
    $article->allComments()->create([
        ...
        'status'         => $article->auto_approve_comments ? 'approved' : 'pending',
    ]);

    return redirect()->back()->with('success', 'Comment submitted and awaiting approval.');
}

    public function approveComment(\App\Models\ArticleComment $comment)
    {
        $comment->approve();
        return redirect()->back()->with('success', 'Comment approved.');
    }

    public function rejectComment(\App\Models\ArticleComment $comment)
    {
        $comment->reject();
