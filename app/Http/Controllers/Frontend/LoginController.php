...
// Check if business profile is rejected
$businessProfile = \App\Models\BusinessProfile::where('user_id', $user->id)->first();
if ($businessProfile && $businessProfile->verification_status === 'rejected') {
    // Verify password first before showing rejection info
    if (!Hash::check($validated['password'], $user->password)) {
        return back()
            ->withErrors(['email' => 'These credentials do not match our records.'])
            ->withInput($request->only('email'));
    }
    // Password correct but rejected — don't login, show rejection details
    return back()
        ->with('vendor_rejected', true)
        ->with('rejection_reason', $businessProfile->rejection_reason)
        ->with('business_name', $businessProfile->business_name)
        ->with('business_profile_id', $businessProfile->id)
...
public function replyToRejection(Request $request)
{
    $request->validate([
        'business_profile_id' => 'required|integer',
        'reason_reply' => 'required|string|max:2000',
    ]);

    $businessProfile = \App\Models\BusinessProfile::findOrFail($request->business_profile_id);
    $businessProfile->update(['reason_reply' => $request->reason_reply]);
    ...
    return back()
        ...
        ->with('business_name', $businessProfile->business_name)
        ->with('business_profile_id', $businessProfile->id)
        ->with('user_name', $businessProfile->user->name ?? '')
        ->withInput($request->only('email'));
}
}
