...
class AddonController extends Controller
{
    ...
    public function create()
    {
        ...
        $locations = [
            'Homepage' => ['herosection', 'featuredsuppliers', 'trendingproducts', 'Weeklyads', 'Popularads', 'Specialads', 'Heroads'],
            'Products' => ['sidebar', 'top_banner', 'bottom_banner', 'ads'],
...
$addon = Addon::create([
        ...
        \App\Models\Notification::create([
            'title'     => 'New Ad Slot Available',
            'content'   => 'A new advertising slot at "' . $addon->locationX . ' → ' . $addon->locationY . '" is now available for vendors to purchase.',
            'link_url'  => '/admin/addons/' . $addon->id,
            'user_id'   => auth()->id(),
            'vendor_id' => null,
            'country_id'=> $addon->country_id ?? null,
            'is_read'   => false,
        ]);
...
$addon->update([
        ...
        // Notify active subscribers of this addon about the update
        $subscriberUserIds = $addon->addonUsers()
        ...
        foreach ($subscriberUserIds as $userId) {
            \App\Models\Notification::create([
                ...
                'content'   => 'The advertising slot you subscribed to ("' . $addon->locationX . ' → ' . $addon->locationY . '") has been updated by the admin. Please review the changes.',
                'link_url'  => '/vendor/addons/' . $addon->id,
                'user_id'   => $userId,
                'vendor_id' => null,
                'country_id'=> $addon->country_id ?? null,
                'is_read'   => false,
...
// Notify all past subscribers before deletion
        $subscriberUserIds = $addon->addonUsers()->pluck('user_id');

        foreach ($subscriberUserIds as $userId) {
            \App\Models\Notification::create([
                'title'     => 'Ad Slot Removed',
                'content'   => 'The advertising slot "' . $addon->locationX . ' → ' . $addon->locationY . '" has been removed from AfriSellers. Contact support for more information.',
                'link_url'  => null,
                'user_id'   => $userId,
                'vendor_id' => null,
                'country_id'=> $addon->country_id ?? null,
