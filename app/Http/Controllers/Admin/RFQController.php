...
class RFQController extends Controller
{
    ...
    public function showVendors(RFQs $rfq)
    {
        try {
            ...
            // Notify the RFQ submitter that admin is reviewing it
            if ($rfq->user_id) {
                $alreadyNotified = \App\Models\Notification::where('user_id', $rfq->user_id)
                    ->where('link_url', '/rfq/' . $rfq->id)
                    ->where('title', 'Your RFQ is Under Review')
                    ->exists();

                if (!$alreadyNotified) {
                    \App\Models\Notification::create([
                        'title'     => 'Your RFQ is Under Review',
                        'content'   => 'Our team is reviewing your request for quotation. You will be notified once vendors respond.',
                        'link_url'  => '/rfq/' . $rfq->id,
                        'user_id'   => $rfq->user_id,
                        'vendor_id' => null,
                        'country_id'=> $rfq->country_id ?? null,
                        'is_read'   => false,
                    ]);
                }
            }
    ...
    public function storeMessage(Request $request, RFQs $rfq)
    {
        try {
            ...
            // Get vendor_id from request if present (to redirect back to vendor messages)
                        ...
                        if ($vendorId) {
                            \App\Models\Notification::create([
                                'title'     => 'New Reply on Your RFQ',
                                'content'   => 'Admin has replied to your message on RFQ: ' . \Illuminate\Support\Str::limit($rfq->message, 60),
                                'link_url'  => '/vendor/rfq/' . $rfq->id,
                                'user_id'   => $vendorId,
                                'vendor_id' => null,
                                'country_id'=> $rfq->country_id ?? null,
                                'is_read'   => false,
