    ...
    public function index(Request $request)
{
    ...
    public function updateStatus(Request $request, Buyer $buyer)
    {
        ...
        try {
            ...
            $statusMessages = [
                'active'    => ['title' => 'Account Activated',  'content' => 'Your buyer account has been activated. You can now browse and purchase on AfriSellers.'],
                'suspended' => ['title' => 'Account Suspended',  'content' => 'Your buyer account has been suspended. Please contact our support team for assistance.'],
                'pending'   => ['title' => 'Account Under Review', 'content' => 'Your buyer account is currently under review. You will be notified once it is approved.'],
            ];

            if (isset($statusMessages[$validated['account_status']]) && $buyer->user_id) {
                $msg = $statusMessages[$validated['account_status']];
                \App\Models\Notification::create([
                    'title'     => $msg['title'],
                    'content'   => $msg['content'],
                    'link_url'  => '/buyer/dashboard',
...
public function verifyEmail(Buyer $buyer)
{
    try {
        Log::info('Verify email attempt', [
            'buyer_id'       => $buyer->id,
            'user_id'        => $buyer->user_id,
            'current_status' => $buyer->email_verified,
            'requested_by'   => auth()->id(),
        ]);

        if ($buyer->email_verified) {
            Log::warning('Verify email skipped — already verified', [
                'buyer_id' => $buyer->id,
            ]);
        ...
        $buyer->update(['email_verified' => true]);

        if ($buyer->user_id) {
            \App\Models\Notification::create([
                'title'      => 'Email Verified',
                'content'    => 'Your email address has been verified by an administrator.',
                'link_url'   => '/buyer/dashboard',
                'user_id'    => $buyer->user_id,
                'vendor_id'  => null,
                'country_id' => $buyer->country_id ?? null,
        ...
        Log::info('Buyer email verified successfully', [
            'buyer_id'     => $buyer->id,
            'verified_by'  => auth()->id(),
        ]);

        return back()->with('success', 'Buyer email verified successfully.');

    } catch (\Exception $e) {
        Log::error('Failed to verify buyer email', [
            'buyer_id' => $buyer->id,
            'error'    => $e->getMessage(),
            'trace'    => $e->getTraceAsString(),
        ]);

        return back()->with('error', 'Failed to verify email. Please try again.');
    }
}
}
